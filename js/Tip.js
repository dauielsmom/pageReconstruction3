(function() {
	function MoveDiv() {
		this.move = function(dom, json) {
			distance = Math.sqrt(Math.pow(json.x2 - json.x1, 2)
					+ Math.pow(json.y2 - json.y1, 2)), s = (json.y2 - json.y1)
					/ distance, c = (json.x2 - json.x1) / distance, tdelay = 0;
			var move = function() {
				tdelay += 3;
				if (tdelay > distance) {
					clearInterval(st);
	...
	Drag.prototype = {
		...
		start : function() {
			this.addEventListen(this.activeDom, 'mousedown',
		...
		getMousedownHandle : function() {
			...
			return function(e) {
				...
				if (srcObj.tagName.toLowerCase() != "img") {
					...
					_this.addEventListen(document, 'mousemove',
							_this.mousemoveHandle);
					_this.addEventListen(document, 'mouseup',
							_this.mouseupHandle);
					...
					document.getElementById("Tip_bg").parentNode
							.removeChild(document.getElementById("Tip_bg"));
					document.getElementById("Tip").parentNode
							.removeChild(document.getElementById("Tip"));
		...
		getMouseupHandle : function() {
			...
			return function(e) {
				...
				_this.removeEventListen(document, 'mousemove',
						_this.mousemoveHandle);
				_this.removeEventListen(document, 'mouseup',
						_this.mouseupHandle);
...
function Tip(json) {
	...
	var h = (document.body.scrollHeight > document.documentElement.scrollHeight ? document.body.scrollHeight
			: document.documentElement.scrollHeight)
			+ "px";
	bg.style.cssText = "width:"
			+ w
			+ "; height:"
			+ h
			+ "; background:#333333;filter:alpha(opacity=50);-moz-opacity:0.5;opacity:0.5;position:absolute;left:0;top:0;z-index:998";
	document.body.appendChild(bg);
	var tip = document.createElement("div");
	tip.id = "Tip";
	var w1 = (window.screen.availWidth - 300) / 2 + "px";
	var h1 = (window.screen.availHeight - 180) / 2 + "px";
	tip.style.cssText = "width:300px; height:180px; background-color:#ffffff;overflow:hidden;position:fixed;z-index:999;left:"
			+ w1 + ";top:" + h1;
	switch (json.pos) {
	case 1:
		var backpos = 0;
	...
	tip.innerHTML = '<div  align="left" id="TipTop" style="height:25px; background-color:#f7f7fa;"><span style="float:left;margin-left:4px;margin-top:2px;display:inline-block;"><b>提示信息</b></span><img id="Tip_im" style="float:right;margin-top:4px;margin-right:4px;cursor:pointer" src="img/close.jpg"></div>'
			+ '<div style="padding-left:20px;padding-top:40px;"></div><div style="margin-top:10px;height:46px;font-weight:bold;font-size:16px;float:left;margin-left:2px;">'
			+ json.str + '</div></div>';
	document.body.appendChild(tip);
	if (!document.getElementById("Tip")) {
		alert(json.str);
	}
	var drag = new Drag(json.callback, document.getElementById('TipTop'),
			document.getElementById('Tip'), json.move);
	drag.start();
}